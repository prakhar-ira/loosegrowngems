...
export function DiamondsCollection({collection}: DiamondsCollectionProps) {
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    const filtered = productsToFilter.filter((product: ProductWithDetails) => {
      ...
      if (filters.shape.length > 0) {
        if (!attributes.shape || !filters.shape.includes(attributes.shape)) {
          return false;
        }
      }
  ...
  return (
    ...
    <div className="diamonds-collection flex flex-col md:flex-row md:items-start gap-6 p-4 md:p-6">
      ...
      {/* Filter Section */}
      {/* Use flex-shrink-0 to prevent the filter panel from shrinking. Adjust width as needed. */}
      {/* Temporarily removed sticky, top-0, and height constraint for debugging */}
      <div className="filters-section w-full md:w-72 lg:w-80 flex-shrink-0 p-4 border rounded-lg shadow-sm md:p-6 md:border-r md:border-gray-200 md:rounded-none md:shadow-none flex flex-col gap-6 md:gap-8 overflow-y-auto hide-scrollbar">
        ...
        <div className="natural-lab-toggle flex border border-gray-300 rounded overflow-hidden">
        ...
        {/* Add more filter groups for shape, color, clarity, cut later */}
      </div>
