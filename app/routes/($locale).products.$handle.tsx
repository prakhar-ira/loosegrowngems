...
export const handle = {
  breadcrumb: (data: { product?: { title: string } }) => data?.product?.title || 'Product',
};
...
function toTitleCase(str: string) {
  return str.split(' ').map(word => {
    ...
    const lowerCaseWords = ['a', 'an', 'and', 'as', 'at', 'but', 'by', 'for', 'in', 'of', 'on', 'or', 'the', 'to', 'via'];
    return word.toLowerCase() === word && lowerCaseWords.includes(word.toLowerCase()) ? 
      word.toLowerCase() : 
...
export default function Product() {
  const { product, diamondAttributes, initialVariant } = useLoaderData<typeof loader>();
  ...
  const [selectedVariantState, setSelectedVariantState] = useState(initialVariant);

  // Effect to update selectedVariantState based on URL search params
  useEffect(() => {
    console.log("PDP Calculating selectedVariant (inside useEffect watching searchParams)...");
    ...
    console.log("PDP useEffect (watching searchParams) - Found variant:", newSelectedVariant);
    ...
    if (urlImageId) {
      ...
      if (foundImage) {
        ...
        return foundImage;
          ...
          console.warn("PDP useMemo - image_id param provided but no matching image found in images.nodes.");
      }
    }
    // Fallback logic (variant -> featured -> first)
    const fallbackImage = selectedVariantState?.image || featuredImage || (images?.nodes && images.nodes[0]);
  ...
  const [currentImage, setCurrentImage] = useState<ImageType | null>(initialImage);
  ...
  const availableImages = useMemo(() => {
    ...
    return images?.nodes?.filter((img: ImageType): img is ImageType => !!img.id && !!img.url) || [];
  ...
  useEffect(() => {
    console.log("PDP Image Update Effect Triggered. selectedVariantState:", selectedVariantState);
    console.log("PDP selectedVariantState Image:", selectedVariantState?.image);
    if (selectedVariantState?.image) {
      console.log("PDP Setting current image to variant image:", selectedVariantState.image);
      ...
      console.log("PDP Selected variant state has no specific image. Current image unchanged by effect.");
    }
      ...
      <div className="w-full">
        ...
        <div className="flex flex-col px-4 my-8 md:my-12">
          ...
          <section className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 lg:gap-16 xl:gap-24">
            ...
            <div className="md:col-span-1 flex gap-4 h-[400px] md:h-[500px] lg:h-[600px]"> {/* Adjust height as needed */}
              {/* Thumbnails Column */}
                ...
                <div className="flex flex-col space-y-2 w-16 flex-shrink-0 overflow-y-auto scrollbar-thin pr-1">
                  {availableImages.map((image: ImageType) => (
                    <button
                      ...
                      className={`block p-0.5 border-1 border-transparent overflow-hidden transition hover:border-slate-400 focus:outline-none focus:border-slate-400 ${currentImage?.id === image.id ? 'border-slate-800' : ''}`}
                    >
                      <Image
                        data={image}
              ...
              <div className="relative flex-grow flex items-center justify-center overflow-hidden border border-gray-200 rounded-lg">
                {currentImage ? (
                  ...
                  <div className="w-full h-full bg-gray-100 flex items-center justify-center text-gray-500 rounded-lg">No Image Available</div>
                )}
              </div>
            </div>
            ...
            <div className="md:col-span-2 flex flex-col justify-between h-full gap-8 md:gap-16 lg:gap-20">
              ...
              <div className="flex flex-col gap-4">
                ...
                <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl 2xl:!text-[80px] font-medium lg:!font-light leading-tight product-title line-clamp-2 overflow-hidden">{formattedTitle}</h1>
                
                {/* Diamond Attributes (Now inside the group) */}
                {(diamondAttributes.color || diamondAttributes.clarity || diamondAttributes.cut) && (
                ...
                <div className="flex flex-wrap gap-4"> { /* Container for Color/Size selectors */ }
                  <VariantSelector
                  ...
                  >
                    {({ option }) => {
                      ...
                      if (option.name.toLowerCase() === 'color') {
                        ...
                        const selectedValue = selectedVariantState?.selectedOptions.find(
                          (opt: SelectedOption) => opt.name.toLowerCase() === 'color',
                        )?.value;
                          ...
                          <div key={option.name} className="">
                            ...
                            <div className="flex flex-wrap border border-gray-300 p-2 w-fit gap-0">
                              {option.values.map(({ value, isAvailable, isActive, to }) => {
                                ...
                                const isManuallyActive = selectedVariantState?.selectedOptions.find((opt: SelectedOption) => opt.name.toLowerCase() === 'color')?.value === value;
                                const colorValue = value.toLowerCase();
                                let swatchStyle: CSSProperties = {};
                                if (colorValue.includes('yellow')) { swatchStyle = { backgroundColor: '#FFD700' }; }
                                else if (colorValue.includes('rose')) { swatchStyle = { backgroundColor: '#B76E79' }; }
                                else if (colorValue.includes('white') || colorValue.includes('silver')) { swatchStyle = { backgroundColor: '#E5E7EB' }; }
                                const textColor = swatchStyle.backgroundColor ? darkenColor(swatchStyle.backgroundColor as string, 0.4) : '#4B5563';
                                const nextSearchParams = new URLSearchParams();
                                // Use selectedVariantState here
                                selectedVariantState?.selectedOptions.forEach((opt: SelectedOption) => {
                                  nextSearchParams.set(opt.name, opt.value);
                                });
                                nextSearchParams.set(option.name, value);
                                const correctTo = `${location.pathname}?${nextSearchParams.toString()}`;
                                return (
                                  <Link
                                    ...
                                    prefetch="intent"
                                    replace
                                    className={`h-8 px-4 flex items-center justify-center text-xs relative border-r border-b border-gray-300 transition-transform duration-100 ease-out ${isManuallyActive ? 'shadow-xl' : ''} ${isAvailable ? 'opacity-100' : 'opacity-50 cursor-not-allowed'}`}
                                    style={swatchStyle}
                                    title={value}
                                    aria-disabled={!isAvailable}
                                  >
                                    <span className={`text-xs font-semibold z-10 ${!swatchStyle.backgroundColor ? 'text-gray-700' : ''}`} style={{ color: textColor }}>
                                      {value.split(' ')[0].replace('k', ' carats')}
                                    </span>
                                    {isManuallyActive && (
                                      <svg className="ml-1 inline" width="10" height="10" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M1 4L3 6L7 1" stroke={textColor} strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/>
                                      </svg>
                                    )}
                                  </Link>
                                );
                      ...
                      const isRingSizeOrLength = ['ring size', 'length'].includes(option.name.toLowerCase());

                      if (isRingSizeOrLength) {
                        // Find the selected value using selectedVariantState
                        const selectedValue = selectedVariantState?.selectedOptions.find(
                          (opt: SelectedOption) => opt.name.toLowerCase() === option.name.toLowerCase(), // Use current option name
                        )?.value;
                        return (
                          <div key={option.name} className=""> {/* Removed mb-4 */}
                            ...
                            <div className="flex flex-wrap gap-0 border border-gray-300 w-fit p-2 overflow-hidden"> 
                              {option.values.map(({ value, isAvailable, isActive, to }) => {
                                // Determine active state manually using selectedVariantState
                                const isManuallyActive = selectedVariantState?.selectedOptions.find((opt: SelectedOption) => opt.name.toLowerCase() === option.name.toLowerCase())?.value === value;
                                const nextSearchParams = new URLSearchParams();
                                // Use selectedVariantState here
                                selectedVariantState?.selectedOptions.forEach((opt: SelectedOption) => {
                                  nextSearchParams.set(opt.name, opt.value);
                                ...
                                return (
                                  <Link
                                    ...
                                    prefetch="intent"
                                    replace
                                    className={`px-4 py-1.5 text-sm text-center border border-gray-300 -mt-px -ml-px transition-transform duration-100 ease-out relative ${isManuallyActive ? 'shadow-xl' : ''} ${isAvailable ? 'opacity-100' : 'opacity-50 cursor-not-allowed'}`}
                                    aria-disabled={!isAvailable}
                                    title={value}
                                  >
                                    {value}
                                    {isManuallyActive && (
                                      <svg className="ml-1 inline" width="10" height="10" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M1 4L3 6L7 1" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/>
                                      </svg>
                                    )}
                                  </Link>
                                );
              ...
              <div className="flex flex-col gap-6 md:gap-10">
                  ...
                  <div className="flex items-center justify-end w-full gap-4">
                    <div className="flex items-center gap-4">
                      <span className="border border-black px-2 sm:px-4 py-1 sm:py-1.5 flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm">
                        <img src="/icons/sourced-with-care-icon.svg" alt="Sourced With Care" className="w-3 h-3 sm:w-4 sm:h-4" />
                        Sourced With Care
                      </span>
                      <span className="border border-black px-2 sm:px-4 py-1 sm:py-1.5 flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm">
                        <img src="/icons/igi-certified-icon.svg" alt="IGI Certified" className="w-3 h-3 sm:w-4 sm:h-4" />
                        IGI Certified
                      </span>
                    </div>
                    <a href="#" className="font-medium hover:underline">
                      <span className="hidden sm:inline text-xs sm:text-sm">DOWNLOAD REPORT</span>
                ...
                <div className="flex flex-col gap-3 md:gap-4 w-full">
                  <div className="flex gap-4 w-full">
                    ...
                    <div className="flex items-center border-0"> {/* Remove border from parent */}
                      {/* Decrease Button */}
                      <button
                      ...
                      {/* Quantity Display */}
                      <span className="w-12 text-center py-2 border-t border-b border-gray-300 text-lg"> {/* Added top/bottom border, adjusted padding/text */} 
                         {quantity}
                      </span>
                      {/* Increase Button */}
                      <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 w-full px-4 sm:px-6 lg:px-8 mb-16">
              ...
              <div className="flex flex-col items-center text-center gap-6">
                <div className="w-16 h-16 flex items-center justify-center">
                  <img src="/assets/images/in_home_mode.svg" alt="Completely In-House" className="w-12 h-12 object-contain" />
                </div>
                <div className="space-y-2">
                  <h3 className="text-xl font-medium">Completely In-House</h3>
                  <p className="text-sm text-gray-600">
                    All our products are made completely in-house by expert craftsmen to ensure the highest degree of beauty and precision.
                  </p>
                </div>
              </div>
              {/* Lifetime Warranty */}
              <div className="flex flex-col items-center text-center gap-6">
                <div className="w-16 h-16 flex items-center justify-center">
                  <img src="/assets/images/card_membership.svg" alt="Lifetime Warranty" className="w-12 h-12 object-contain" />
                </div>
                <div className="space-y-2">
                  <h3 className="text-xl font-medium">Lifetime Warranty</h3>
                  <p className="text-sm text-gray-600">
                    Complete Carat Lifetime Warranty is limited to the repair and/or replacement at Complete Carat's discretion.
                  </p>
                </div>
              </div>
              {/* Priority Shipping */}
              <div className="flex flex-col items-center text-center gap-6">
                <div className="w-16 h-16 flex items-center justify-center">
                  <img src="/assets/images/priority_shipping.png" alt="Priority Shipping" className="w-12 h-12 object-contain" />
                </div>
                <div className="space-y-2">
                  <h3 className="text-xl font-medium">Priority Shipping</h3>
                  <p className="text-sm text-gray-600">
                    Purchase is inclusive of shipping insurance. We use UPS Priority Shipping to make sure you receive your item safely and quickly.
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-col items-center text-center gap-6">
                <div className="w-16 h-16 flex items-center justify-center">
                  <img src="/assets/images/partner_exchange.svg" alt="Conflict-Free Diamonds" className="w-12 h-12 object-contain" />
                </div>
                <div className="space-y-2">
                  <h3 className="text-xl font-medium">Conflict-Free Diamonds</h3>
                  <p className="text-sm text-gray-600">
...
function parseDiamondAttributes(description: string | null | undefined): {
  ...
  const attributes: { color: string | null; clarity: string | null; cut: string | null } = {
    color: null, 
    clarity: null, 
    cut: null 
  };
  ...
  const colorRegex = new RegExp(`(?:colour|color)\s*:\s*(.*?)(?=\s*(?:${nextKeywords})\s*:|$)`, 'i');
  ...
  const clarityRegex = new RegExp(`clarity\s*:\s*(.*?)(?=\s*(?:${nextKeywords})\s*:|$)`, 'i');
  ...
  const cutRegex = new RegExp(`cut\s*:\s*(.*?)(?=\s*(?:${nextKeywords})\s*:|$)`, 'i');
  ...
  if (cutMatch && cutMatch[1]) {
    ...
    attributes.cut = (value.length > 0 && value.toUpperCase() !== 'N/A') ? value : null;
  }
...
export async function loader({ params, context, request }: LoaderFunctionArgs) {
  ...
  const initialVariant = product.variants.nodes.find((variant: ProductVariant) =>
    variant.selectedOptions.every(
      (option: SelectedOption) => searchParams.get(option.name) === option.value,
    ),
  ) || product.variants.nodes[0];

  // --- Parse description for diamond attributes ---
  let diamondAttributes: { color: string | null; clarity: string | null; cut: string | null } = {
    color: null,
    clarity: null,
    cut: null
  };
